...
ipcMain.handle("get-customers", async (_event, params?: PaginationParams) => {
  try {
...
ipcMain.handle("delete-customers", async (_event, id: number) => {
  try {
...
ipcMain.handle("get-suppliers", async (_event, params?: PaginationParams) => {
  try {
...
ipcMain.handle(
  "update-suppliers",
  async (_event, { id, data }: { id: number; data: UpdateSupplierPayload }) => {
    try {
...
ipcMain.handle("delete-suppliers", async (_event, id: number) => {
  try {
...
ipcMain.handle("get-dashboard-data", async (_event) => {
  try {
...
ipcMain.handle("get-products", async (_event, params?: PaginationParams) => {
...
ipcMain.handle(
  "update-products",
  async (_event, { id, data }: { id: number; data: UpdateProductPayload }) => {
    const res = await axios.put(`${API_URL}/products/${id}`, data);
    return res.data as ProductType;
  },
);

ipcMain.handle("delete-products", async (_event, id: number) => {
...
ipcMain.handle("delete-suppliers-payments", async (_event, id: number) => {
  await axios.delete(`${API_URL}/suppliers-payments/${id}`);
  return true;
});

ipcMain.handle("list-sales", async (_, params: PaginationParams) => {
  try {
    const res = await axios.get(`${API_URL}/sales`, { params });
    return res.data; // O backend retornará o array ou objeto paginado
  } catch (error) {
    return handleAxiosError(error);
  }
});
...
// ipcMain.handle("delete-sale", async (_, id: number) => {
//   try {
...
ipcMain.handle("get-sale-by-id", async (_, id: number) => {
  try {
